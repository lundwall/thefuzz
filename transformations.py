...
class NoTransformation(BaseTransformation):
    def __init__(self):
        super().__init__("No transformation")
...
class PrependDotSlashTransformation(BaseTransformation):
    ...
    def transform(self, test: BaseModuleTest):
        ...
        test.replace_in_code_with(self.original, "./" + self.original)


class FilenameTransformation(BaseTransformation):
    def __init__(self, original, new):
        super().__init__(
            f"Replace '{original}' with '{new}' in filenames, foldernames and code"
        )
        self.original = original
        self.new = new
...
class SnapshotTransformation(BaseTransformation):
    def __init__(self):
